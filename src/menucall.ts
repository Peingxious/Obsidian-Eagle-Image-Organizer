...
export function registerEscapeButton(
	plugin: MyPlugin,
	menu: Menu,
...
export async function addEagleImageMenuPreviewMode(
	plugin: MyPlugin,
	menu: Menu,
	oburl: string,
	event: MouseEvent,
	...
	if (imageInfo) {
		const { id, name, ext, annotation, tags, url, folders } =
			imageInfo as any;
		const comments = (imageInfo as any).comments as
			| { id?: string; duration?: number; annotation?: string }[]
			| undefined;
		...
		menu.addItem((item: MenuItem) => {
			...
			item.setTitle(t("menu.openFileSubmenu"))
			...
			subMenu.addItem((subItem) =>
				subItem
					...
					.onClick(async (event: MouseEvent) => {
						// 根据设置决定如何打开链接
						const openMethod =
							plugin.settings.openInObsidian || "newPage";
			...
			subMenu.addItem((subItem) =>
				subItem
					...
					.onClick(() => {
						...
						const child = spawn("explorer.exe", [localFilePath], {
							shell: true,
						});
						child.on("error", (error) => {
						...
						child.on("exit", (code) => {
							if (code === 0) {
								...
								print(
									`The file cannot be opened normally, exit code: ${code}`,
								);
							}
						});
					}),
			...
			subMenu.addItem((subItem) =>
				subItem
					...
					.onClick(() => {
						...
						const child = spawn(
							"rundll32",
							["shell32.dll,OpenAs_RunDLL", localFilePath],
							{ shell: true },
						);
		...
		menu.addItem((item: MenuItem) => {
			...
			item.setTitle(t("menu.openComments"))
				.setIcon("quote-glyph")
				.onClick(defaultAction);
			...
			if (hasComments) {
				...
				comments!.forEach((c, index) => {
					const label =
						c.annotation && c.annotation.trim().length > 0
							? c.annotation
							: `#${index + 1}`;
					subMenu.addItem((subItem) =>
						subItem.setTitle(label).onClick(() => {
							openComment();
						}),
					);
				});
			...
			const defaultAction = () => {
				...
				try {
					if (editor) {
						...
						if (editorView.dom.contains(target)) {
							...
							print(
								"DEBUG: Calculated target_pos before modal:",
								saved_target_pos,
							);
						} else {
							print(
								"DEBUG: Target is not in editor DOM (Reading View?)",
							);
							// 尝试直接使用光标位置（如果是编辑模式）
				...
				new ModifyPropertiesModal(
					...
					(newId, newName, newAnnotation, newUrl, newTags) => {
						try {
							print(
								"DEBUG: ModifyPropertiesModal callback triggered",
							);
							// 2. 使用提前计算的位置
							if (saved_target_pos !== -1) {
								print(
									"DEBUG: Updating link with saved pos:",
									saved_target_pos,
								);
								updateCurTargetLinkTitle(
									oburl,
									plugin,
									saved_target_pos,
									newName,
								...
								print(
									"DEBUG: No saved pos, trying to recalculate...",
								);
								const target = getMouseEventTarget(event);
								const editor =
									plugin.app.workspace.getActiveViewOfType(
								...
								const editorView = (editor as any)
									.cm as EditorView;

								try {
									const current_pos =
										editorView.posAtDOM(target);
									print(
										"DEBUG: Recalculated pos in callback:",
										current_pos,
									);
									updateCurTargetLinkTitle(
										oburl,
									...
									print(
										"DEBUG: Failed to recalculate pos:",
										e,
									);
							...
							print(
								"DEBUG: Error in ModifyPropertiesModal callback:",
								e,
							);
		...
		menu.addItem((item: MenuItem) => {
			...
			item.setTitle(t("menu.copyDataSubmenu"))
				.setIcon("copy")
				.onClick(defaultAction);
					...
					subItem
						...
						.onClick(() => {
							try {
								...
								const target_line =
									editorView.state.doc.lineAt(target_pos);
								...
								if (finds.length === 1) {
									...
									const end = finds[0][1];
									const linkText = line_text.slice(
										start,
										end,
									);
									navigator.clipboard.writeText(linkText);
									new Notice(
										t("menu.copyToClipboardSuccess"),
									);
			...
			subMenu.addItem((subItem) =>
				subItem
					.setIcon("letter-text")
					.setTitle(
						t("menu.eagleAnnotation", {
							annotation:
								annotation.length > 14
									? annotation.substring(0, 14) + "..."
									: annotation,
						}),
					)
					.onClick(() => {
			...
			subMenu.addItem((subItem) =>
				subItem
					.setIcon("tags")
					.setTitle(
						t("menu.eagleTags", { tags: tagsArray.join(", ") }),
					)
					.onClick(() => {
						...
						navigator.clipboard
							...
							.catch(
								(err) => new Notice(t("menu.copyTagsFailed")),
							);
					}),
			);
...
function copyFileToClipboardCMD(filePath: string) {
	...
	const callback = (error: Error | null, stdout: string, stderr: string) => {
		if (error) {
			new Notice(
				t("menu.commandError", { message: error.message }),
				3000,
			);
	...
	if (process.platform === "darwin") {
		...
		exec(
			`powershell -command "Set-Clipboard -Path '${safeFilePath}'"`,
			callback,
		);
	}
}
...
// 查找一行中包含特定URL的链接
export function findLinkInLine(url: string, line: string): [number, number][] {
