...
export class FolderSelectModal extends Modal {
    ...
    async onOpen() {
        ...
        tabsRow.style.cssText = 'display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;';
        this.tabsContainer = tabsRow;

        this.folderListEl = contentEl.createDiv({ cls: 'eagle-folder-list' });
        this.folderListEl.style.cssText = 'max-height:300px;overflow-y:auto;border:1px solid var(--background-modifier-border);padding:10px;margin-top:10px;';
    ...
    private buildRootTabs() {
        ...
        if (rootsConfig.length > 0) {
            ...
            if (scopeId) {
                ...
                if (scopedRoot?.children?.length) baseRoots = [scopedRoot];
            }
            for (const root of baseRoots) {
                if (root.children) tabs.push(...root.children);
            }
        }

        this.rootTabs = tabs;
        if (this.activeRootId && !this.rootTabs.find(f => f.id === this.activeRootId)) this.activeRootId = null;
        if (!this.activeRootId && this.rootTabs.length > 0) this.activeRootId = this.rootTabs[0].id;
    }
    ...
    private renderTabs() {
        ...
        const createChip = (text: string, rootId: string | null) => {
            ...
            chip.style.cssText = 'padding:2px 8px;border-radius:999px;cursor:pointer;font-size:12px;border:1px solid var(--background-modifier-border);';

            const isActive = rootId === null ? this.activeRootId === null : this.activeRootId === rootId;
            if (isActive) {
        ...
        for (const tab of this.rootTabs) createChip(tab.name, tab.id);
    }
    ...
    renderFolderList() {
        ...
        if (rootsConfig.length > 0) {
            if (this.activeRootId) {
                ...
                roots = rootsConfig.map(cfg => this.findFolderById(cfg.folderId!, this.folders)).filter((n): n is EagleFolder => !!n);
            }
        }

        const renderTree = (nodes: EagleFolder[], depth: number, container: HTMLElement) => {
             if (this.searchQuery && depth === 0) {
                 const collectMatches = (n: EagleFolder, list: EagleFolder[]) => {
                     if (n.name.toLowerCase().includes(this.searchQuery)) list.push(n);
                     if (n.children) n.children.forEach(c => collectMatches(c, list));
                 };
                 const matches: EagleFolder[] = [];
                 nodes.forEach(n => collectMatches(n, matches));
                 matches.forEach(m => this.renderFolderItem(m, 0, container));
                 return;
             }

             for (const node of nodes) {
                 this.renderFolderItem(node, depth, container);
                 if (node.children) renderTree(node.children, depth + 1, container);
             }
        };
        renderTree(roots, 0, this.folderListEl);
    }

    private renderFolderItem(node: EagleFolder, depth: number, container: HTMLElement) {
        const div = container.createDiv({ cls: 'eagle-folder-item' });
        div.style.cssText = `padding-left:${depth * 20 + 5}px;display:flex;align-items:center;`;
    ...
    async save() {
        try {
            ...
            if (folders.length === 0 && this.activeRootId) folders = [this.activeRootId];
            const libraryPath = this.plugin.settings.libraryPath;

            for (const id of this.itemIds) {
                const metaPath = path.join(libraryPath, 'images', `${id}.info`, 'metadata.json');
