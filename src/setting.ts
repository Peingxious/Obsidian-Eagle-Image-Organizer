...
export class SampleSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const librariesTitleDiv = librariesTextWrapper.createDiv({
			cls: "setting-item-name",
		});
		librariesTitleDiv.textContent = t("setting.libraries.title");

		const librariesDescDiv = librariesTextWrapper.createDiv({
			cls: "setting-item-description",
		});
		librariesDescDiv.textContent = t("setting.libraries.desc", {
		...
		const addLibraryButton = librariesHeaderRow.createEl("button", {
			text: t("setting.libraries.addLibrary"),
		});
		...
		libraries.forEach((lib, index) => {
			...
			if (index === 0) {
				...
				libContainer.style.backgroundColor =
			...
			header.addExtraButton((button) => {
				const isActive =
					lib.id === this.plugin.settings.currentLibraryId;
				button.setIcon(isActive ? "check" : "arrow-right");
				button.setTooltip(
					isActive
						? t("setting.libraries.active")
						: t("setting.libraries.setActive"),
				);
				if (isActive) {
			...
			lib.paths.forEach((p, pathIndex) => {
				new Setting(libContainer)
					...
					.addText((text) =>
						text
							.setPlaceholder(
								t("setting.libraryPaths.pathPlaceholder"),
							)
							.setValue(p)
							.onChange(async (value) => {
		...
		const idTitleDiv = idTextWrapper.createDiv({
			cls: "setting-item-name",
		});
		idTitleDiv.textContent = t("setting.group.id.title");

		const idDescDiv = idTextWrapper.createDiv({
			cls: "setting-item-description",
		});
		...
		projectRoots.forEach((root, index) => {
			const row = new Setting(projectContainer).setClass(
				"eagle-path-setting",
			);
			row.addText((text) => {
				text.setPlaceholder(
					t("setting.projectFolderRoots.namePlaceholder"),
				);
			...
			row.addText((text) => {
				text.setPlaceholder(
					t("setting.projectFolderRoots.idPlaceholder"),
				);
			...
			const infoEl = rowEl.querySelector(
				".setting-item-info",
			) as HTMLElement | null;
			if (infoEl) {
		...
		const filterTitle = filterTextWrapper.createDiv({
			cls: "setting-item-name",
		});
		filterTitle.textContent = t("setting.folderFilter.title");
		const filterDesc = filterTextWrapper.createDiv({
			cls: "setting-item-description",
		});
		filterDesc.textContent = t("setting.folderFilter.desc");

		const addFilterButton = filterHeader.createEl("button", {
			text: t("setting.folderFilter.add"),
		});
		...
		filters.forEach((filter, index) => {
			const row = new Setting(filterContainer).setClass(
				"eagle-path-setting",
			);
			row.addText((text) => {
			...
			row.addExtraButton((button) => {
				...
				if (active) {
					...
					button.extraSettingsEl.removeClass(
						"eagle-subfolder-active",
					);
				}
				button.onClick(async () => {
		...
		const imageTitleDiv = imageTextWrapper.createDiv({
			cls: "setting-item-name",
		});
		imageTitleDiv.textContent = t("setting.group.image.title");

		const imageDescDiv = imageTextWrapper.createDiv({
			cls: "setting-item-description",
		});
		...
		new Setting(imageBody)
			...
			.addToggle((toggle) => {
				toggle
					.setValue(this.plugin.settings.clickView)
					.onChange(async (value) => {
		...
		new Setting(imageBody)
			...
			.addDropdown((dropdown) => {
				dropdown
					.addOption("newPage", t("setting.openInObsidian.newPage"))
					.addOption("popup", t("setting.openInObsidian.popup"))
					.addOption(
						"rightPane",
						t("setting.openInObsidian.rightPane"),
					)
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) => {
				toggle
					.setValue(this.plugin.settings.websiteUpload)
					.onChange(async (value) => {
		...
		new Setting(containerEl)
			.setName(t("setting.reverseSyncOnOpen.name"))
